...
export class PrioritizedFileObjectLoader {
    public static loadFromFiles<T>(
        defaultObject: T,
        filePaths: string[],
        descriptionForWarnings: string,
    ): Promise<{ loadedObject: T; path: string | undefined }> {
        return PrioritizedFileObjectLoader._loadFromFiles(defaultObject, filePaths, descriptionForWarnings, true).then(
            (result: { loadedObject: T | undefined; path: string | undefined }) => {
                if (!result.loadedObject) {
                    throw new Error(
                        "PrioritizedFileObjectLoader.loadFromFiles: result is null. Maybe the default object was null?",
                    );
                }
    ...
    public static loadFromFilesNoMerge<T>(
        defaultObject: T,
        filePaths: string[],
        descriptionForWarnings: string,
    ): Promise<{ loadedObject: T | undefined; path: string | undefined }> {
        return PrioritizedFileObjectLoader._loadFromFiles(defaultObject, filePaths, descriptionForWarnings, false);
    }

    private static _loadFromFiles<T>(
        defaultObject: T,
        filePaths: string[],
        descriptionForWarnings: string,
        merge: boolean = true,
            ...
            for (const filePath of filePaths) {
                if (fs.existsSync(filePath)) {
                    ...
                    try {
                        ...
                        Log.log().debug(
                            `Could not read ${descriptionForWarnings} (${filePath}): ${error.message}`,
                        );
                    ...
                    try {
                        ...
                        Log.log().debug(
                            `Failed to parse ${descriptionForWarnings} (${filePath}) as YAML: ${error.message}`,
                        );
                    }
                    // try to load as YAML:
                    try {
                        ...
                        throw new Error(
                            `The ${descriptionForWarnings} file (${filePath}) is malformed.` +
                            `Failed to parse it as JSON or YAML.`,
                        );
                    }
                }
            }
